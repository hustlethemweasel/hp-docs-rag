...
- **Metrics (RAGAS-style, LLM-as-judge pinned to `claude-sonnet-5` regardless of `LLM_PROVIDER`, so per-provider scores share one grader and are comparable — a judge that swaps with the generator would score each provider on its own curve, and a judge that is also a contestant would grade its own answers, a documented self-preference bias. The judge is deliberately stronger than every graded model; requires `ANTHROPIC_API_KEY` even for local-provider runs):**
...
- **Reproducibility:** benchmark runs pin `temperature=0` on generation and rewrite calls (and fixed seeds where the provider supports them) so tuning comparisons are apples-to-apples rather than sampling noise. Judge calls omit temperature entirely — the parameter is removed on Claude 5-family models and `claude-sonnet-5` rejects it with a 400. (The original harness passed the generation provider to the judge, so the first published anthropic numbers were haiku grading its own answers. Before the first Ollama run the judge was pinned to `claude-sonnet-5` — stronger than either contestant, self-judging for neither — and both providers were re-run under the fixed harness so every column shares one scorer, including the refusal-phrase detector, which gained a phrasing observed live from qwen3.5:4b.)
