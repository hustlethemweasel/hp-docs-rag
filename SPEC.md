...
- **Dense leg is the cross-lingual path — measured degraded on first turns.** The embedding model (harrier) is multilingual, so a non-English query embeds near the English chunks it's about, and a pt-BR smoke test (2026-07-19) confirms topically correct chunks surface (paper-jam procedures, the E3 error table). But the corpus's own foreign-language regulatory pages (a Brazilian e-waste notice on the ENVY guide's p. 127, Spanish/French radio notices pp. 117-118) outrank them at position 1-2 on *every* pt-BR query tried — language affinity beats topic in this embedding space, so 2-3 of the six context slots go to junk on first-turn pt-BR queries (later turns are rescued by the English rewrite). Exact tokens in the query (E3, part numbers) still light up the sparse leg language-independently. **Open decision, gated on the pt-BR eval slice:** always-rewrite (translate turn one too, at +1 LLM call of latency) vs. accepting degraded first-turn context.
