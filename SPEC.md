...
| R4 | Cloud or local models | Provider abstraction: `anthropic` / `ollama`, chosen via env var | Done — `ChatProvider` protocol + `AnthropicProvider`/`OllamaProvider`/`ScriptedProvider`, factory reads `LLM_PROVIDER` (M3); `scripted` serves the fast suite and load-test scenario (a) (M6) |
...
| R8 | Search strategy | Hybrid retrieval: dense (cosine) + sparse (Postgres FTS), fused with RRF (§8) | Done — `HybridRetriever` runs dense + sparse top-20, fuses with RRF (k=60), caps at top-6, refuses on empty retrieval (M3) |
...
| R13 | Benchmark response quality | Golden Q&A set + RAGAS-style metrics with LLM-as-judge (§13) | Done — 50-question golden set (`eval/golden.jsonl`) across factual/procedure/figure/multi-turn/negative/exact-token/pt-BR categories; `eval/run.py` scores faithfulness, answer relevancy, context precision/recall, and refusal correctness against the real HybridRetriever + the configured provider, judged by pinned `claude-sonnet-5`; per-provider comparison in `eval/REPORT.md` — claude-haiku-4-5: refusal accuracy 0.980, faithfulness 0.857, relevancy 0.952, context recall 0.932; qwen3.5:4b (local): refusal accuracy 0.940, faithfulness 0.726, relevancy 0.879, context recall 0.932; the pt-BR slice is 1.000 refusal / 1.000 context recall on both providers (§9.4) |
...
- **Local serving via Ollama, not vLLM.** Optimizes for the "runs anywhere via `docker compose up`" goal: a quantized model on laptop CPU or Apple Silicon with one `ollama pull`, plus a trivial VLM API for figure captioning (§7.4). vLLM wins on concurrent throughput, but that targets a load profile this project doesn't serve (§12) and effectively requires a CUDA GPU. On GPU hardware with real concurrency it's a drop-in via the provider abstraction (§9.3) — vLLM speaks the OpenAI wire protocol, so `VLLMProvider` is a thin httpx client.
...
- **Query rewriting:** before retrieval, a lightweight LLM call condenses the history + new question into a standalone search query **in English** — the corpus language — resolving pronouns and implicit references (so "how do I clean it?" retrieves printhead-cleaning chunks after a printer discussion, and a Portuguese follow-up still lights up both retrieval legs, see §9.4). Runs on **every turn, including the first**: turn one was originally skipped to save an LLM call, but the pt-BR smoke test (§9.4) showed first-turn non-English retrieval loses 2-3 of its 6 context slots to the corpus's foreign-language notice pages, and translating the query is the fix — one extra lightweight call per conversation buys clean retrieval in every language, English included (the rewrite is near-identity there).
...
- **Retrieval is always in English — the rewrite translates.** The embedding model (harrier) is multilingual, and a pt-BR smoke test (2026-07-19) confirmed topically correct chunks surface for raw pt-BR queries (paper-jam procedures, the E3 error table) — but the corpus's own foreign-language regulatory pages (a Brazilian e-waste notice on the ENVY guide's p. 127, Spanish/French radio notices pp. 117-118) outranked them at position 1-2 on *every* pt-BR query tried: language affinity beats topic in this embedding space, costing 2-3 of the six context slots. **Resolution: query rewriting runs on every turn including the first (§9.2)**, so retrieval always sees an English query and both legs behave exactly as measured on the English golden set; cross-lingual embedding quality is no longer load-bearing. Exact tokens (E3, part numbers) additionally light up the sparse leg language-independently.
...
- **Evidence status: measured (2026-07-19).** An 8-case pt-BR golden slice (six answerable mirrors including one multi-turn, two negatives expecting Portuguese-phrased refusals) runs through the full quality benchmark: refusal accuracy 1.000 and context recall 1.000 on **both** providers — the English rewrite carries retrieval, and qwen's lower pt-BR faithfulness (0.650 vs haiku 0.883) matches its English gap rather than a language effect. Building the slice also caught two qwen3.5:4b rewrite-prompt defects (ignored a parenthetical "in English"; hallucinated "[Part Number]" placeholders from the token-preservation clause) — the rewrite prompt is now worded to what the 4B model actually obeys. Details in `eval/REPORT.md`'s multilingual round section.
...
Implementations: `AnthropicProvider`, `OllamaProvider`, and `ScriptedProvider` — a genuine implementation of the protocol that streams pre-scripted responses with configurable latency. Per the constitution (§2), it carries no fake/mock naming because it honors the full interface and real streaming behavior; it serves the fast test suite and load-test scenario (a) in §12. A factory reads `LLM_PROVIDER` + `LLM_MODEL` env vars. (A never-implemented `openai` option was removed — R4 needs one cloud and one local provider. Adding a provider is a three-step change: implement the class, extend the `Literal`, wire the factory branch.)
...
- **Metrics (RAGAS-style; LLM-as-judge pinned to `claude-sonnet-5` — one grader for every provider, stronger than any model under test, so no self-judging; requires `ANTHROPIC_API_KEY` even for local runs):**
...
- **Reproducibility:** `temperature=0` on generation and rewrite calls; judge calls omit it (the parameter is removed on Claude 5-family models).
...
**Resolved** (details in the referenced sections): conversations scoped by a client-generated `X-User-Id` UUID (§6, §10); minimal GitHub Actions CI in scope (§11); cross-encoder re-ranker rejected — the spike showed a wash on the golden set, code removed and restorable from commit `f1b92a9` (§8, `eval/REPORT.md`); the unimplemented `openai` provider option and the `REFUSAL_THRESHOLD` knob removed (§9.3, §8).
